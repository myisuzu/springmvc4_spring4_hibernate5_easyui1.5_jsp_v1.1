	...
	$('.uploder-container').each(function(index){
        ...
        uploader[index]	.on('ready',function(){
        	...
        	if (input_hidden_id != null && input_hidden_id != "") {
        		...
        		var fileUrlStr = "";
        		if (index == 0) {
        			//fileIdStr = $("#input_hidden_image1").val();
        			fileUrlStr = $("#input_hidden_image1").val();
        		} else if (index == 1) {
        			//fileIdStr = $("#input_hidden_image2").val();
        			fileUrlStr = $("#input_hidden_image2").val();
        		}
	        	...
	        	$.ajax({
	        		...
	        		success: function(files){
	        			//说明请求第一个图片域
	        			if (index == 0) {
	                		//$("#myForm").append("<input type='hidden' name='image1' value='"+files[0].id+"'>");
	        				$("#myForm").append("<input type='hidden' name='image1' value='"+files[0].url+"'>");
	                	}
	        			//说明请求第二个图片域
	                	if (index == 1) {
	                		for(var i = 0; i < files.length; i++){
	                			//$("#myForm").append("<input type='hidden' name='image2' value='"+files[i].id+"'>");
	                			$("#myForm").append("<input type='hidden' name='image2' value='"+files[i].url+"'>");
	                		}
	                	}
	        			for(var i = 0; i < files.length; i++){
	        				var obj ={};
        ...
        uploader[index].on('uploadSuccess',function(file,reponse){
        	...
        	if (index == 0) {
        		...
        		//$("#myForm").append("<input type='hidden' name='image1' value='"+reponse.data.fileId+"'>");
        		$("#myForm").append("<input type='hidden' name='image1' value='"+reponse.data.fileUrl+"'>");
        	}
        	//第二个图片域
        	if (index == 1) {
        		//$("#myForm").append("<input type='hidden' name='image2' value='"+reponse.data.fileId+"'>");
        		$("#myForm").append("<input type='hidden' name='image2' value='"+reponse.data.fileUrl+"'>");
        	}
        	//alert("图片上传成功！");
        });
